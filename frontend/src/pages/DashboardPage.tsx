  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-presight-primary">Dashboard</h1>
          <p className="text-sm text-slate-500">Latest evaluation health by domain</p>
        </div>
      </div>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {summaries.map(({ domain, run }) => (
          <div key={domain} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-700">{domain}</h2>
              <span className="text-xs uppercase text-slate-400">{run ? 'Latest run' : 'No runs yet'}</span>
            </div>
