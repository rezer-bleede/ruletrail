  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-presight-primary">Runs</h1>
          <p className="text-sm text-slate-500">Review historical evaluations</p>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-200">
            {runs.map((run) => (
              <tr key={run.id} className="hover:bg-slate-50">
                <td className="px-4 py-3 font-medium text-slate-700">#{run.id}</td>
                <td className="px-4 py-3">{run.domain}</td>
